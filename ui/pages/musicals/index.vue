...
const { data, pending } = useLazyAsyncData('musicalsByPreference', () => {
  return GqlMusicalsByPreference({preference: preferences}, { accept: 'application/json' })
}, { server: false, initialCache: false });
watch(data, (value) => {
  musicals.value = value.musicalsByPreference;
}, { deep: true });
...
const handleUnsorted = async () => {
  const { musicalsByPreference } = await GqlMusicalsByPreference({preference: preferences}, { accept: 'application/json' })
  musicals.value = musicalsByPreference;
}
